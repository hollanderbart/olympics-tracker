...
- **Data Fetching:** Client-side with auto-refresh intervals
- **API:** Proxy endpoints to olympics.com JSON data
...
```
/app
  /api          - API routes (Edge runtime, proxy olympics.com endpoints)
...
/components
  ...
  MedalOverview.tsx    - Dutch medal count with animations
  MedalTally.tsx       - Full medal table (expandable)
  EventList.tsx        - Dutch events schedule with countdowns
  LoadingState.tsx     - Loading skeletons and states

/lib
  olympics.ts    - Core data fetching logic
  types.ts       - TypeScript type definitions
  constants.ts   - App constants (refresh intervals, API endpoints, sport icons)
...
- **lib/constants.ts** - Sport icons (⛸️ 🏒 🛷 💀 💃), refresh intervals, API URLs
...
- **Sport Icons:** Use emoji icons from constants.ts (⛸️ ice skating, 🏒 hockey, 🛷 sliding, 💀 skeleton, 💃 figure skating, etc.)
...
### Building
```bash
npm run pages:build  # Build for Cloudflare Pages (creates .vercel/output)
```
...
### Environment
- **Local:** Standard Next.js dev server
- **Production:** Cloudflare Pages with Edge runtime
